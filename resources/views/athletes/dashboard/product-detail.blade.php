<x-app-layout>
    ...
    <div class="w-full row">
        <div class="col-md-8">
            <p class="mb-0 dashboard-product-detail-title"><a href="{{route('athletes.dashboard')}}" class="text-decoration-none">Dashboard</a> / Product Details</p>
    ...
    <div class="w-full row product-detail">
        ...
        <div class="col-md-6 product-content-detail">
            ...
            <p class="title">{{ $product->title }}
                <span class="float-right">{{ $product->currency }} {{ $product->getPrice() }}</span>
            </p>
            <p class="description">{{ $product->description }}</p>
            <p class="product-variants">Select Size</p>
            <div class="flex">
                @foreach ($variants as $variant)
                    <div class="product-variant-container{{!empty($cartItem) && $cartItem->variant == $variant ? ' active' : '' }}">{{ $variant }}</div>
                @endforeach
                <input type="hidden" name="cart-variant" required value="{{$cartItem->variant ??  (isset($variants[0]) ?? '') }}">
                <input type="hidden" name="product-uuid" value="{{ $product->uuid }}">
            </div>
            <span id="variant-error"></span>
            ...
            <p class="mt-4 product-variants">Quantity</p>
            <p class="flex qty">
                <button type="button" class="qtyplus" aria-hidden="true">
                ...
                <input type="number" name="cart-quantity" id="qty" min="1" max="10"
                    step="1" value="{{$cartItem->quantity ?? 1 }}">
                <button type="button" class="qtyminus" aria-hidden="true"><svg width="24" height="24"
    ...
    <div class="w-full mt-6 dashboard-navbar">
        <div class="cart-list-container">
            ...
            @include('athletes.dashboard.list', [
                ...
                'route_name' => 'athlete.product.detail'
            ])
        </div>
    </div>
    @push('script')
